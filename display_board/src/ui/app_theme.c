 ...
 *      DEFINES
 *********************/
#if !defined(LV_DPX_CALC)
#define LV_DPX_CALC    _LV_DPX_CALC 
#endif
struct _my_theme_t;
typedef struct _my_theme_t my_theme_t;
my_theme_t *theme_def = NULL;
...
static void style_init(my_theme_t * theme)
{
    ...
    lv_style_set_bg_opa(&theme->styles.label, LV_OPA_TRANSP);
    ...
    lv_style_set_bg_color(&theme->styles.bg_color_primary_muted, lv_color_white());
...
lv_theme_t * app_default_theme_init(lv_display_t * disp) {
    return app_theme_init(disp, lv_color_hex(0x3d3c4f), lv_color_hex(0x1a1a1b),
...
static void theme_apply(lv_theme_t * th, lv_obj_t * obj)
{
    ...
    if(lv_obj_check_type(obj, &lv_obj_class)) {
        ...
        lv_obj_add_style(obj, &theme->styles.scrollbar, LV_PART_SCROLLBAR);
        lv_obj_add_style(obj, &theme->styles.scrollbar_scrolled, LV_PART_SCROLLBAR | LV_STATE_SCROLLED);
    }  // end if(lv_obj_check_type(obj, &lv_obj_class))
#if LV_USE_BUTTON
