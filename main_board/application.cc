...
main_api_t __main_api = {
    .voice_start = voice_start,
    .voice_end = voice_end,
    .get_i2s_tx_handle = get_i2s_tx_handle,
    .get_i2s_std_config = []() -> i2s_std_config_t* {
        return Board::GetInstance().GetAudioCodec()->GetStdConfig();
    },
};

i2s_chan_handle_t get_i2s_tx_handle() {
    return Board::GetInstance().GetAudioCodec()->GetTxHandle();
}
...
void Application::StartListening() {
    ...
    if (!protocol_) {
        ESP_LOGE(TAG, "StartListening: Protocol not initialized");
        return;
    }

    Board::GetInstance().GetAudioCodec()->ReconfigI2S();
...
void Application::MainEventLoop() {
    while (true) {
        ...
        auto bits = xEventGroupWaitBits(event_group_, SCHEDULE_EVENT, pdTRUE, pdFALSE, pdMS_TO_TICKS(10));
