...
NoAudioCodecSimplex::NoAudioCodecSimplex(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, gpio_num_t mic_sck, gpio_num_t mic_ws, gpio_num_t mic_din) {
    ...
    // 接收通道配置（INMP441）
    // INMP441 输出特性：
    // 数据格式：24 位左对齐（有效数据在最高 24 位，低 8 位无意义）。
    // 每个样本占用 32 位槽位（实际只使用高 24 位）。
    // ESP32 I2S 配置目标：
    // 使用 32 位帧 接收数据，直接保留原始 24 位数据（无需位偏移）。
    std_cfg = {
        .clk_cfg = {
            .sample_rate_hz = (uint32_t)input_sample_rate_, // 确保使用输入采样率
            .clk_src = I2S_CLK_SRC_DEFAULT,
            .mclk_multiple = I2S_MCLK_MULTIPLE_256,
        },
        .slot_cfg = {
            .data_bit_width = I2S_DATA_BIT_WIDTH_32BIT,  // 接收32位帧
            .slot_bit_width = I2S_SLOT_BIT_WIDTH_32BIT,  // 槽位宽度32位
            .slot_mode = I2S_SLOT_MODE_MONO,
            .slot_mask = I2S_STD_SLOT_LEFT,            // 左声道
            .ws_width = I2S_DATA_BIT_WIDTH_32BIT,       // WS信号宽度匹配
            .ws_pol = false,                            // WS低电平有效
            .bit_shift = false,                         // 禁用位偏移（关键！）
                                                        // INMP441 的数据是左对齐的（有效数据在最高 24 位），禁用位偏移可保留原始数据位置。
                                                        // 若启用 bit_shift，ESP32 会尝试将数据右对齐，导致采样错位（表现为音频变慢/失真）。
            .left_align = true                          // 左对齐（关键！）强制硬件直接存储原始左对齐数据，确保高 24 位有效数据完整存入 32 位帧。
        },
        .gpio_cfg = {
            .mclk = I2S_GPIO_UNUSED,
            .bclk = mic_sck,
    ...
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(rx_handle_, &std_cfg));
    ESP_LOGI(TAG, "Simplex channels created #1");
}
