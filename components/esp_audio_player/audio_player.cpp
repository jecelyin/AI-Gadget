...
typedef enum {
    ...
    AUDIO_PLAYER_REQUEST_SEEK,               /**< seek to a position in the audio file */
...
#endif
    uint16_t current_position;  // Current playback position in milliseconds
    uint16_t total_duration;    // Total audio duration in milliseconds
} audio_instance_t;
            ...
            if (AUDIO_PLAYER_REQUEST_PAUSE == audio_event.type) {
                ...
                while(1) {
                    ...
                    if((AUDIO_PLAYER_REQUEST_PLAY != audio_event.type) &&
                       ...
                       (AUDIO_PLAYER_REQUEST_SEEK != audio_event.type) &&
...
#if defined(CONFIG_AUDIO_PLAYER_ENABLE_MP3)
            case FILE_TYPE_MP3:
                if (AUDIO_PLAYER_REQUEST_SEEK == audio_event.type || audio_event.position > 0) {
                    if(!mp3_seek(i->mp3_decoder, fp, &i->mp3_data, audio_event.position)) {
                        ESP_LOGE(TAG, "mp3 seek failed");
                    }
                    audio_event.type = AUDIO_PLAYER_REQUEST_PLAY;
                    audio_event.position = 0; // reset position after seek
                }
                decode_status = decode_mp3(i->mp3_decoder, fp, &i->output, &i->mp3_data, frame_info);
                mp3_decode = (MP3DecInfo *)i->mp3_decoder;
                instance.total_duration = total_bytes * 8 / mp3_decode->bitrate; // in milliseconds
                total_frames++; // 帧数+1
                
                // 计算当前播放位置（秒）
                current_time = (total_frames * frame_info.outputSamps) / mp3_decode->samprate;
                instance.current_position = (int)current_time;
                ESP_LOGD(TAG, "mp3 current position %d, total duration %d",
                    instance.current_position, instance.total_duration);
                break;
#endif
#if defined(CONFIG_AUDIO_PLAYER_ENABLE_WAV)
            case FILE_TYPE_WAV:
                if (AUDIO_PLAYER_REQUEST_SEEK == audio_event.type || audio_event.position > 0) {
                    if(!wav_seek(fp, &i->wav_data, audio_event.position)) {
                        ESP_LOGE(TAG, "wav seek failed");
                    }
                    audio_event.type = AUDIO_PLAYER_REQUEST_PLAY;
                    audio_event.position = 0; // reset position after seek
                }
                decode_status = decode_wav(fp, &i->output, &i->wav_data);
                instance.current_position = get_wav_current_position(fp, &i->wav_data);
                instance.total_duration = get_wav_duration(&i->wav_data);
                ESP_LOGD(TAG, "wav current position %d, total duration %d",
                    instance.current_position, instance.total_duration);
                break;
#endif
...
esp_err_t audio_player_play(FILE *fp, uint16_t position)
{
    LOGI_1("%s", __FUNCTION__);
    audio_player_event_t event = { .type = AUDIO_PLAYER_REQUEST_PLAY, .fp = fp, .position = position };
 ...
 */
esp_err_t audio_player_seek(uint16_t position)
{
    LOGI_1("%s", __FUNCTION__);
    audio_player_event_t event = { .type = AUDIO_PLAYER_REQUEST_SEEK, .position = position };
    return audio_send_event(&instance, event);
}
/**
...
uint16_t audio_player_get_current_position() {
    return instance.current_position;
}

uint16_t audio_player_get_total_duration() {
