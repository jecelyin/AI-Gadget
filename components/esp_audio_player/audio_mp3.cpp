...
#include "audio_mp3.h"
/**
 * some function include from library https://github.com/chmorgan/esp-libhelix-mp3
 * @link
 */
static const char *TAG = "mp3";

bool is_mp3(FILE *fp, mp3_instance *pInstance) {
 ...
 */
DECODE_STATUS decode_mp3(HMP3Decoder mp3_decoder, FILE *fp, decode_data *pData, mp3_instance *pInstance, MP3FrameInfo &frame_info) {
...
bool mp3_seek(HMP3Decoder mp3_decoder, FILE *fp, mp3_instance *pInstance, uint16_t position)
{
    if (!fp || !mp3_decoder || !pInstance) {
        return false;
    }
    ...
    // 重置文件指针到开头，解码第一帧以获取比特率等信息
    fseek(fp, 0, SEEK_SET);
    uint8_t header_buf[MAINBUF_SIZE];
    size_t n_read = fread(header_buf, 1, MAINBUF_SIZE, fp);
    if (n_read < 4) {
        return false;
    }

    // 寻找帧同步字
    int offset = MP3FindSyncWord(header_buf, n_read);
    if (offset < 0 || (offset + 4) > n_read) {
        return false;
    }

    // 获取第一帧的信息（包括比特率）
    MP3FrameInfo frame_info;
    if (MP3GetNextFrameInfo(mp3_decoder, &frame_info, header_buf + offset) != 0) {
        return false;
    }

    // 计算跳转的目标位置（按字节计算）
    int bitrate = frame_info.bitrate; // bps
    ...
    // duration = file_size * 8 / bitrate, position 秒占比：
    float duration = (float)file_size * 8 / bitrate;
    if (position >= duration) {
        position = (uint16_t)(duration - 1); // 最多跳转到末尾前1秒
    }

    int target_byte = (int)((float)bitrate * position / 8.0f);
    if (target_byte >= file_size) {
        target_byte = file_size - 1;
    }

    // 安全起见，回退一定字节防止跳过帧头
    int seek_start = target_byte - 1024;
    if (seek_start < 0) {
        seek_start = 0;
    }

    fseek(fp, seek_start, SEEK_SET);
    n_read = fread(header_buf, 1, MAINBUF_SIZE, fp);
    if (n_read < 4) {
        return false;
    }

    offset = MP3FindSyncWord(header_buf, n_read);
    if (offset < 0 || (offset + 4) > n_read) {
        return false;
    }

    int final_seek = seek_start + offset;
    fseek(fp, final_seek, SEEK_SET);

    // 重置解码器实例状态
    pInstance->bytes_in_data_buf = 0;
    pInstance->read_ptr = pInstance->data_buf;
    pInstance->eof_reached = false;
