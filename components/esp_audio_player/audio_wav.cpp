...
bool is_wav(FILE *fp, wav_instance *pInstance) {
    ...
    while(true) {
        ...
        {
            pInstance->data_start_pos = ftell(fp); // 记录 data 起始位置
            pInstance->data_size = subchunk.SubchunkSize; // 记录 data 大小
            // 计算 BytesPerSecond（优化 Seek 计算）
            pInstance->bytes_per_second = 
                pInstance->header.SampleRate * 
                pInstance->header.NumChannels * 
                (pInstance->header.BitsPerSample / 8);
            break;
        } else {
...
bool wav_seek(FILE *fp, wav_instance *pInstance, float target_time) {
    uint32_t target_byte_offset = (uint32_t)(target_time * pInstance->bytes_per_second);
    if (target_byte_offset >= pInstance->data_size) {
        target_byte_offset = pInstance->data_size - 1;
    }
    
    long target_pos = pInstance->data_start_pos + target_byte_offset;
    return (fseek(fp, target_pos, SEEK_SET) == 0);
}

float get_wav_duration(wav_instance *pInstance) {
    wav_header_t *wav = &pInstance->header;
    uint32_t bytes_per_sample = wav->BitsPerSample / 8;
    uint32_t total_samples = pInstance->data_size / (wav->NumChannels * bytes_per_sample);
    return (float)total_samples / wav->SampleRate;
}

float get_wav_current_position(FILE *fp, wav_instance *pInstance) {
    wav_header_t *wav = &pInstance->header;
    long current_pos = ftell(fp);
    uint32_t bytes_consumed = current_pos - pInstance->data_start_pos;
    
    uint32_t bytes_per_second = wav->SampleRate * wav->NumChannels * (wav->BitsPerSample / 8);
    return (float)bytes_consumed / bytes_per_second;
}
